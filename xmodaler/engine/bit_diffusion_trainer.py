...
class BitDiffusionTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        """
        try:
            data = next(self._train_data_loader_iter) #D:\python_work\ImageCaption\SCD\SCD-Net-main\SCD-Net-main\xmodaler\datasets\images\mscoco_diffusion.py line 132
        ...
        data = comm.unwrap_model(self.model).preprocess_batch(data) #对数据进行扩充5倍，因为每张图片对应5个caption
        if self.debug:
            origin_targets = data[kfg.U_TOKENS_IDS]
            origin_targets_str = self.model.beam_searcher.output_sents(origin_targets.view(-1, 20))

        outputs_dict = self.model(data) #SCD-Net-main/xmodaler/modeling/meta_arch/bit_diffusion_ref_transformer_enc_dec.py  line 72
