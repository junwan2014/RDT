...
class ScheduledSampling(HookBase):
    ...
    def after_step(self, start_time, best_cider):
...
class IterationTimer(HookBase):
    ...
    def after_train(self, start_time):
...
class PeriodicCheckpointer(_PeriodicCheckpointer, HookBase):
    ...
    def after_step(self, start_time, best_cider):
        ...
        self.step(iteration = self.trainer.iter, epoch=(self.trainer.iter+1) // self.trainer.iters_per_epoch, best_cider=best_cider)
...
class EvalHook(HookBase):
    ...
    def _do_eval(self, epoch, start_time=None, best_cider=None, mode='EMA' ):#测试输出
        if self._multi_gpu_eval:
            results = self._func(epoch)
        else:
            if comm.is_main_process():
                results, mode = self._func(epoch)
        et = time.time() - start_time
        et = str(datetime.timedelta(seconds=et))[:-7]
        if mode=='EMA':
            tmp = '{:.6f}'.format( results['CIDEr'])
            tmp = float(tmp)
            best_cider[1] = tmp
        time_tmp = "{} ".format(et)
        head = "Epoch {} - ".format(epoch)
        if mode == 'train':
            log = "Val  scores, 'CIDEr': [{:.6f}], 'BLEU': [{:.6f}, {:.6f}, {:.6f}, {:.6f}], 'METEOR': [{:.6f}], 'ROUGE': [{:.6f}].".format(
                results['CIDEr'], results['BLEU'][0], results['BLEU'][1], results['BLEU'][2], results['BLEU'][3],
                results['METEOR'], results['ROUGE'])
        else:
            log = "EMA  scores, 'CIDEr': [{:.6f}], 'BLEU': [{:.6f}, {:.6f}, {:.6f}, {:.6f}], 'METEOR': [{:.6f}], 'ROUGE': [{:.6f}].".format(
                results['CIDEr'], results['BLEU'][0], results['BLEU'][1], results['BLEU'][2], results['BLEU'][3],
                results['METEOR'], results['ROUGE'])
        log_tmp = time_tmp + head + log
        print(log_tmp)
    ...
    def after_step(self, start_time, best_cider):
        # self._do_eval(0) 测试调用
        next_iter = self.trainer.iter + 1
        epoch = int(next_iter // self.trainer.iters_per_epoch)
        # self._do_eval(epoch,start_time,  best_cider)
        if (self._period > 0) and (next_iter % self._period == 0) and (epoch > self._eval_start):
        # if (self._period > 0) and ((next_iter) % 200 == 0) and (epoch > self._eval_start):
            self._do_eval(epoch, start_time, best_cider)
