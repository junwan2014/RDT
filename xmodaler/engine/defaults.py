...
""",
    ...
    # parser.add_argument("--config-file", default="./configs/image_caption/scdnet/stage1/diffusion.yaml", metavar="FILE", help="path to config file")
    #强化学习调用
    # parser.add_argument("--config-file", default="./configs/image_caption/scdnet/stage1/diffusion_rl.yaml", metavar="FILE",
...
class DefaultTrainer(TrainerBase):
    ...
    def resume_or_load(self, resume=True):
        ...
        if (not resume) and self.cfg.MODEL.USE_EMA:
    ...
    def build_hooks(self):
        ...
        def test_ema_and_save_results(epoch):
            ...
            if isinstance(self.ema, ModelEma):
                ...
                eval_results = self.test(self.cfg, self.ema, self.test_data_loader, self.test_evaluator, epoch)
                mode = 'EMA'
        ...
        if self.test_data_loader is not None:
            ...
            if comm.is_main_process(): #调整checkpoint执行顺序
                ret.append(
    ...
    @classmethod
    def test(cls, cfg, model, test_data_loader, evaluator, epoch):#测试函数主体
        model.eval()
        results = []
        with torch.no_grad():
