...
class MSCoCoDiffusionDataset(MSCoCoDataset):
    ...
    def __call__(self, dataset_dict):
        ret = super().__call__(dataset_dict) # D:\python_work\ImageCaption\SCD\SCD-Net-main\SCD-Net-main\xmodaler\datasets\images\mscoco.py line 113
        ...
        u_tokens_ids = [torch.roll(item, -1, dims=0).long() for item in ret[kfg.G_TOKENS_IDS]] #所有的词向量向前滚动一个位置
        u_ret = {
...
class MSCoCoDiffusionRefDataset(MSCoCoDiffusionDataset):
    ...
    def __call__(self, dataset_dict):
        ...
        if self.stage != 'train':
            ...
            sent_num = len(dataset_dict['r_tokens_ids']) #20个句子随机选择5个
