...
class BitEmbedding(TokenBaseEmbedding):
    ...
    def forward(self, batched_inputs):
        ...
        u_tokens_ids = batched_inputs[kfg.U_TOKENS_IDS] #真实标记向左移一位，其余位置为0
        u_tokens_ids_bit = batched_inputs[kfg.U_TOKENS_IDS_BIT] #U_TOKENS_IDS加噪声之后的结果
    ...
    def _forward(self, input_ids, input_ids_bit, time_info, self_cond=None, token_type_ids=None, c_tokens_ids_bit=None):
        ...
        input_ids_bit = torch.cat((self_cond, input_ids_bit), dim = -1) # 160x20x28
        if c_tokens_ids_bit is not None:
            input_ids_bit = torch.cat((input_ids_bit, c_tokens_ids_bit), dim=-1)

        embeddings = self.embeddings(input_ids_bit) # 160x20x512
