...
__all__ = ["TokenBaseEmbedding", "RefTokenBaseEmbedding"]
...
class TokenBaseEmbedding(nn.Module):
    ...
    def _forward(self, input_ids, token_type_ids=None, time_step=None):
        embeddings = self.embeddings(input_ids) #64x20->64x20x512
        
        if self.embeddings_pos is not None:
            pos_inputs = input_ids if time_step is None else time_step
            position_embeddings = self.embeddings_pos(pos_inputs) #1x20x512
        ...
        if (self.embeddings_token_type is not None) and (token_type_ids is not None):
            ...
            embeddings_token_type = self.embeddings_token_type(token_type_ids) #64x20x512
            embeddings = embeddings + embeddings_token_type

        if self.embeddings_act is not None: #不执行
...
@EMBEDDING_REGISTRY.register()
class RefTokenBaseEmbedding(TokenBaseEmbedding):
    @configurable
    def __init__(
        self, 
        *,
        ...
        **kwargs
        ...
        super(RefTokenBaseEmbedding, self).__init__(**kwargs)

    def forward(self, batched_inputs):
        ret = {}

        r_tokens_ids = batched_inputs[kfg.R_TOKENS_IDS]
        r_tokens_type = batched_inputs.get(kfg.R_TOKENS_TYPE, None)
        r_token_embed = self._forward(r_tokens_ids, token_type_ids=r_tokens_type)
        ret.update({ kfg.R_TOKEN_EMBED: r_token_embed })
        return ret
