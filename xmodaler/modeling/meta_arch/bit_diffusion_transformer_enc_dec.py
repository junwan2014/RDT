...
class BitDiffusionTransformerEncoderDecoder(TransformerEncoderDecoder):
    ...
    def _forward(self, batched_inputs):
        ...
        ve_out = self.visual_embed(batched_inputs) #xmodaler\modeling\embedding\visual_embed.py  line 79
        inputs.update(ve_out)

        if self.encoder is not None:
            encoder_out_v = self.encoder(inputs, mode='v') # xmodaler\modeling\encoder\transformer_encoder.py line 45
    ...
    def noise_sample(self, inputs):
        ...
        # sample random times 随机生成160（批数）个0-0.999之间的数，采用均匀分布 t' in Eq. 1
        times = torch.zeros((batch_size,), device = device).float().uniform_(0, 0.999)

        bit_token_embed = inputs[kfg.U_TOKENS_IDS_BIT] #160x20x14
        noise = torch.randn_like(bit_token_embed) #epsilon in Eq. 1 返回一个和输入大小相同的张量，其由均值为0、方差为1的标准正态分布填充
        #log_snr 对应 gamma(t') in Eq. 1
        noise_level = self.log_snr(times)
        padded_noise_level = right_pad_dims_to(bit_token_embed, noise_level) #160->view(160x1x1)
        alpha, sigma =  log_snr_to_alpha_sigma(padded_noise_level) # 从 noise 那里去生成 alpha 和均值量
        # alpha = sqrt(sigmoid(-gamma(t')))  sigma = sqrt(sigmoid(gamma(t')))
    ...
    def preprocess_batch(self, batched_inputs):
        super_ret = super().preprocess_batch(batched_inputs) #对数据进行预处理

        sample_per_sample = batched_inputs[0].get(kfg.SAMPLE_PER_SAMPLE, 1)
        ret = {}

        if kfg.C_TOKENS_IDS in batched_inputs[0]: #这里不走
        ...
        if kfg.RL_SAMPLE_FIX_TOKENS_IDS in batched_inputs[0]:#这里不走
