...
class BaseEncoderDecoder(nn.Module, metaclass=ABCMeta):
    ...
    def preprocess_batch(self, batched_inputs):
        ...
        vfeats, vmasks = pad_tensor(vfeats, padding_value=0, use_mask=True) # 不足50的特征进行补齐，并将补齐的位置设置为0
        ret = { kfg.ATT_FEATS: vfeats, kfg.ATT_MASKS: vmasks } #ret存放补齐之后的特征和mask

        if kfg.ATT_FEATS_WO_MASK in batched_inputs[0]: #这里不走
        ...
        if kfg.RELATION in batched_inputs[0]: #这里不走
        ...
        if kfg.ATTRIBUTE in batched_inputs[0]: #这里不走
        ...
        if kfg.GLOBAL_FEATS in batched_inputs[0]: #这里不走
        ...
        if kfg.U_TOKENS_IDS in batched_inputs[0]:
            u_tokens_ids = [x[kfg.U_TOKENS_IDS] for x in batched_inputs] #16张图片对应16个list,每张图片对应5个kokens，向量化的句子
            if sample_per_sample > 1:
                u_tokens_ids = flat_list_of_lists(u_tokens_ids)
            u_tokens_ids, tmasks = pad_tensor(u_tokens_ids, padding_value=0, use_mask=True) #对于token_ids进行补齐，并补齐对应的标记，
            ret.update( { kfg.U_TOKENS_IDS: u_tokens_ids, kfg.TOKENS_MASKS: tmasks} ) #放到ret里面

        if kfg.U_TOKENS_IDS_WO_MASK in batched_inputs[0]:  #这里不走
        ...
        if kfg.G_TOKENS_IDS in batched_inputs[0]:  #这里不走
        ...
        if kfg.U_TARGET_IDS in batched_inputs[0]: #这里不走
        ...
        if kfg.G_TARGET_IDS in batched_inputs[0]: #这里不走
        ...
        if kfg.ATT_FEATS_LOC in batched_inputs[0]: #这里不走
        ...
        if kfg.U_TOKENS_TYPE in batched_inputs[0]:
            ...
            u_tokens_type = pad_tensor(u_tokens_type, padding_value=0, use_mask=False) # u_tokens_type进行补齐
            ret.update({ kfg.U_TOKENS_TYPE: u_tokens_type }) #放到ret里面

        if kfg.G_TOKENS_TYPE in batched_inputs[0]: #这里不走
        ...
        if kfg.V_TARGET in batched_inputs[0]:#这里不走
        ...
        if kfg.V_TARGET_LABELS in batched_inputs[0]: #这里不走
        ...
        if kfg.ITM_NEG_LABEL in batched_inputs[0]:#这里不走
        ...
        ####################################### COSNet #######################################
        if kfg.SEMANTICS_IDS in batched_inputs[0]:#这里不走
        ...
        if kfg.SEMANTICS_LABELS in batched_inputs[0]:#这里不走
        ...
        if kfg.SEMANTICS_MISS_LABELS in batched_inputs[0]:#这里不走
        ...
        if kfg.SEQ_PER_SAMPLE in batched_inputs[0]:
            batch_size, max_feats_num, feats_dim = vfeats.size()[0:3] #16x50x2048
            ...
            vfeats = vfeats.reshape(-1, max_feats_num, feats_dim) #根据每张图片有5个caption，对特征进行扩充，变成 80x50x2048
            vmasks = vmasks.unsqueeze(1).expand(batch_size, repeat_num, max_feats_num)
            vmasks = vmasks.reshape(-1, max_feats_num) #对vmasks进行扩充 80x50
            ret.update({ kfg.ATT_FEATS: vfeats, kfg.ATT_MASKS: vmasks }) #更新ret
        ...
        if kfg.IDS in batched_inputs[0]:
            ...
            ret.update({ kfg.IDS: ids }) #这里对图像编号进行扩充
    ...
    def decode_beam_search(self, batched_inputs, output_sents=False):
        return self.beam_searcher( batched_inputs, output_sents, model=weakref.proxy(self))
