...
class DecodeStrategy(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, batched_inputs, output_sents, model):
        ...
        if output_sents:
            if self.vocab:
                sents = decode_sequence(self.vocab, ret[kfg.G_SENTS_IDS]) #转化为真实的句子
            else:
                sents = decode_sequence_bert(self.bert_tokenizer, ret[kfg.G_SENTS_IDS], self.eos_token_id)
            ret.update({ kfg.OUTPUT: sents })
        return ret

    def output_sents(self, g_sents_ids, vocab_w_novel_objs=None):
        if self.vocab:
            sents = decode_sequence(self.vocab if vocab_w_novel_objs is None else vocab_w_novel_objs, g_sents_ids)
        else:
            sents = decode_sequence_bert(self.bert_tokenizer, g_sents_ids, self.eos_token_id, self.coco2gpt)
        return sents