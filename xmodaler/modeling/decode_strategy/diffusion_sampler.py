    ...
    def ddpm_sample(self, model, shape, inputs):
        ...
        for time, time_next in time_pairs:
            ...
            if self.apply_sigmoid_to_pred:
                ...
                x_start.clamp_(-self.bit_scale, self.bit_scale) #把范围压缩到-1和1之间

            # get log(snr)
            log_snr = self.log_snr(time)  # gamma(t_now) gamma(s')
            log_snr_next = self.log_snr(time_next) # gamma(t_next) gamma(t')
            log_snr, log_snr_next = map(partial(right_pad_dims_to, txt), (log_snr, log_snr_next)) # reshape

            # get alpha sigma of time and next time log_snr = gamma(s') log_snr_next=gamma(t'), alpha= alpha_s, sigma = sigma_s, alpha_next = alpha_t, sigma_next = sigma_t
            alpha, sigma = log_snr_to_alpha_sigma(log_snr)
            alpha_next, sigma_next = log_snr_to_alpha_sigma(log_snr_next)

            # derive posterior mean and variance -expm1(gamma(s')-gamma(t'))
            c = -expm1(log_snr - log_snr_next)   # Computes the exponential of the elements minus 1: y_{i} = e^{x_{i}} - 1
            # alpha_next = alpha_s, txt = x_t, alpha_t = alpha, x_start = f(x_t, gamma(t'), V) 上一轮的输出
