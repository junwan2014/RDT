...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, *, num_hidden_layers: int, bert_layers,
        ...
        # self.MLP = nn.Sequential(nn.Linear(512 * num_hidden_layers, 512 * num_hidden_layers), nn.LeakyReLU(),
        #                          nn.Linear(512 * num_hidden_layers, 512), nn.LeakyReLU())
    ...
    #     ret = {}
    #     outs = []
    #     out = batched_inputs[kfg.ATT_FEATS]
    #     if mode == None or mode == 'v':
    #         vfeats = batched_inputs[kfg.ATT_FEATS]
    #         ext_vmasks = batched_inputs[kfg.EXT_ATT_MASKS]
    #         for layer_module in self.layers:
    #             vfeats, _ = layer_module(vfeats, ext_vmasks) # xmodaler\modeling\layers\bert.py line 357
    #             outs.append(vfeats)
    ...
    def forward(self, batched_inputs, mode=None):
        ...
        if mode == None or mode == 'v':
            ...
            ext_vmasks = batched_inputs[kfg.EXT_ATT_MASKS]
            for layer_module in self.layers:
                vfeats, _ = layer_module(vfeats, ext_vmasks, mode)  # xmodaler\modeling\layers\bert.py line 357
            ret.update({kfg.ATT_FEATS: vfeats})
        return ret