...
class PeriodicEpochCheckpointer(PeriodicCheckpointer):
    def step(self, iteration: int, epoch: int, best_cider: float,**kwargs: Any) -> None:
        """
        ...
        if float(best_cider[1])>float(best_cider[0]): #保存模型
        # if float(best_cider[1]) > float(best_cider[0]):  # 保存模型
            best_cider[0] = best_cider[1]
            self.checkpointer.save(
                "{}_Epoch_{:05d}_Iter_{:07d}".format(best_cider[0], epoch, iteration), **additional_state
            )
